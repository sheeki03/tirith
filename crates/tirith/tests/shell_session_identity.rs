...
fn finish(mut startup: Startup, deadline: Instant, fresh_hook: bool) -> String {
    let status = loop {
    ...
    if !fresh_hook {
        assert_eq!(
            ...
            "inactive source changed the caller session"
        );
...
fn check(family: &str, file: &str) {
    ...
    // noninteractive process. Its real interactive session row is exercised by
    // the native PowerShell qualification fixture, not this startup-only case.
    let fresh_hook = family != "pwsh";
    let identities: HashSet<_> = starts
        .into_iter()
        .map(|child| finish(child, deadline, fresh_hook))
        .collect();
    assert_eq!(identities.len(), if fresh_hook { 8 } else { 1 });
}
...
#[test]
fn powershell_noninteractive_source_preserves_the_parent_session() {
