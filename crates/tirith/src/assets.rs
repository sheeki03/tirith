...
// Installed by `setup_cline` only on Windows, but also rendered by a
// cross-platform test, so it is compiled in everywhere and this allow keeps the
// non-Windows non-test build quiet.
