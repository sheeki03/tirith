...
mod native {
    ...
    fn require_procfs(file: &File) -> Result<(), ClockRefusal> {
        ...
        // statfs.f_type is unsigned on musl ARM and signed on GNU. Widen both
        // native values losslessly, without an ambiguous inferred cast.
        if i128::from(unsafe { fs.assume_init() }.f_type) != i128::from(libc::PROC_SUPER_MAGIC) {
            return Err(ClockRefusal::Unsupported);
        }
        Ok(())
    }
