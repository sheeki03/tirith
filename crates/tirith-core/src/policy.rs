...
pub struct ScanPolicyConfig {
    ...
    /// Trusted MCP server policy identities generated by `tirith mcp policy
    /// init`. Each `mcp:v1:<sha256>` key binds source config, declared name, and
    /// transport. A listed identity suppresses `mcp_insecure_server` /
    ...
    /// Legacy bare names intentionally match nothing.
    #[serde(default)]
    pub trusted_mcp_servers: Vec<String>,
    /// Per-server allowed-tools gate (keys are source-qualified policy
    /// identities; values the exact permitted tool names). Both effects flow
    /// through `mcp_server_drift`
...
impl Policy {
    ...
    /// * `scan.trusted_mcp_servers` — listing an exact server identity silences
...
"#;
    ...
    fn f9_sanitize_classification_is_exhaustive() {
        ...
        assert_eq!(
            ...
            "RESET: scan.trusted_mcp_servers (a listed identity silences MCP findings)"
        );
        assert_eq!(
