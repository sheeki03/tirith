...
fn read_license_key() -> Option<String> {
    ...
    let content = match std::fs::read_to_string(&path) {
        ...
        Err(e) => {
            eprintln!(
                "tirith: cannot read license key from {}: {e}",
                path.display()
            );
            return None;
        }
...
fn decode_tier(key: &str) -> Option<Tier> {
    ...
    if let Some(exp_str) = payload.get("exp").and_then(|v| v.as_str()) {
        let exp_date = match chrono::NaiveDate::parse_from_str(exp_str, "%Y-%m-%d") {
            Ok(d) => d,
            Err(e) => {
                eprintln!("tirith: license key has malformed expiry date: {e}");
                return None;
            }
        };
