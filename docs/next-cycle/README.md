...
| WP17 | Bounded CLI/CPU/RSS/service sampling and instrumented allocation counting are implemented. Three exact-class source-6b release100 references and twelve controlled growth/control runs are retained. Only the URL-first growth pair meets cohort admission; five CPU-mismatched pairs remain refused. An explicit macOS ARM collector and twelve controls now support three independent baselines and six same-runner growth pairs. Limits remain disabled until actual comparable growth evidence is reviewed; final-feature, whole-hook/host and concurrent workloads remain. |
