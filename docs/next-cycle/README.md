...
| WP10 | Combined personal profile/shell setup is registered through `setup recommended` and browser plans; the complete browser run passed profile/shell apply and undo. Narrow macOS ARM Claude setup is integrated; actual combined publication, all nine native hook cases, MCP-only control and retained-host reload observations passed on 39f6acc. Other selected agents remain unavailable until qualified. Multi-host qualification and current-shell verification remain. |
...
| WP17 | Bounded CLI/CPU/RSS/service sampling and instrumented allocation counting are implemented. Three exact-class source-6b release100 references and twelve controlled growth/control workflow runs are retained or being evaluated. Limits remain disabled until native growth detection is accepted; final-feature, whole-hook/host and concurrent workloads remain. |
...
| WP25 | Optional bring-your-own server, authenticated authority/client protocol, explicit local enrollment, reviewed CLI/browser publication and bounded rollback are implemented. Thirty-five real HTTPS/browser cases cover exact-intent lost-response recovery, stale reports, partial adoption and offline withdrawal. Linux/Windows client qualification and final-candidate evidence remain; publication never proves fleet adoption. |
| WP26 | LocalLeafMaterializeV1 implements bounded local dependency-free/script-free npm artifact materialization, fresh signed threat/policy/task checks, immutable review journals and explicit recovery/undo. Native Linux writer/recovery qualification remains. No package code executes and the earlier npm/Python private-input execution paths remain disabled; broader installation/containment qualification is still required. |
