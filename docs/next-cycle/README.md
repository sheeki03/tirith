...
| WP17 | Bounded CLI/CPU/RSS/service sampling and instrumented allocation counting are implemented. Three independent Apple M1 (Virtual) baselines pass on 415565d3, with identical allocation counts and bounded RSS spread. All six later native control/growth pairs qualify their byte detectors on that same historical source; the earlier five CPU-mismatched pairs remain refused. Draft thresholds received independent review; limits remain disabled pending final-source measurements and compatibility review. The retained September 21 macOS release product also passes 600 measured native Zsh allow/block frames across one- and two-terminal workloads, beyond the prior receipt limit. Final-source baselines, detector qualification and wider host workloads remain. |
...
and benchmarks. The newer npm integration requires its own checks. These are source/debug
