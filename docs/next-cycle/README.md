...
| WP04 | Shared native shell/operator targets and evidence grades are implemented. Five actual caller-shell helper/PTY checks passed on the retained macOS candidate. Configured and inherited states do not imply observed blocking. Native Windows/PowerShell/Nushell qualification remains. |
...
| WP10 | Combined personal profile/shell setup is registered through `setup recommended` and browser plans; the complete browser run passed profile/shell apply and undo. Selected agents refuse until their combined automatic setup scope is certified. Multi-host qualification and current-shell verification remain. |
...
| WP14 | Six embedded pages expose typed shell/profile/settings/exception plans, saved jobs, feedback, support preview/download, retention, impact reviews and lifecycle actions. Eighteen complete real-browser workflows and five separate delayed-response cases passed on retained candidate a70b8eba, including busy-history paging, combined setup, archive controls and stale-dialog isolation. Final release recertification remains. |
