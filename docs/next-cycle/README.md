...
| WP25 | Bounded impact reports, exception ownership/expiry and local activation/undo are registered through CLI/browser with immutable journal attachments and shared freshness checks. Local CLI and browser activation/undo checks passed. Remote publication/fleet adoption remain explicitly unavailable without a server authority/adoption protocol. |
| WP26 | LocalLeafNoScriptsV1 defines a pinned, zero-transitive local artifact contract. Retained archive capture, quarantine staging, typed runtime-pack/receipt models and shared checkpoint publication are registered and covered by the latest core suite. Actual Node launcher integration and native execution qualification remain; installation is disabled. The family review also disabled Python private-input execution at five public/internal launch boundaries; candidate 1a72833 passed the refusal and no-effects checks. Complete input-lifetime protection remains unqualified. |
