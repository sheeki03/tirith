...
| WP11 | Packaged-byte runner passed 32 native Bash/Zsh/Fish checks on the corrective macOS debug candidate; four additional terminal/npm-launcher checks passed on that candidate. Nine actual Claude configured-hook and boundary controls passed on the later a70b8eba candidate. Other native installed packages/hosts and final candidate recertification remain. |
...
| WP17 | Bounded CLI/CPU/RSS/service sampling and instrumented allocation counting are implemented. Three exact-class source-6b release100 references and twelve controlled growth/control runs are retained. Only the URL-first growth pair meets cohort admission; five CPU-mismatched pairs remain refused. An explicit macOS ARM collector and fifteen controls now support three independent baselines and six same-runner growth pairs. Four initial runs refused the CPU label before measurement; the new exact Apple M1 (Virtual) cohort requires fresh baselines. Limits remain disabled until actual comparable growth evidence is reviewed; final-feature, whole-hook/host and concurrent workloads remain. |
...
Native ARM GNU/musl and all six PR release target builds pass on `c3445508`;
see [verification](verification.md) for remaining test and npm assembly corrections.
